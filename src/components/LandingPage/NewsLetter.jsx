...
const fadeUp = {
  ...
  visible: (i = 0) => ({ opacity: 1, y: 0, transition: { delay: i * 0.15, duration: 0.5, ease: [0.22, 1, 0.36, 1] } }),
};
...
export default function Newsletter() {
  ...
  return (
    <section className="w-full flex items-center justify-center bg-white px-4 py-6">
      <div className="w-full max-w-lg flex flex-col items-center text-center">
        <motion.p variants={fadeUp} initial="hidden" animate="visible" custom={0} className="text-[10px] text-gray-500 tracking-wide mb-1">
          Our Newsletter
        </motion.p>
        <motion.h2 variants={fadeUp} initial="hidden" animate="visible" custom={1} className="font-[poppins] text-sm sm:text-base font-bold text-black leading-snug mb-4">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={2} className="w-full">
          <div className="flex w-full items-stretch rounded-xl overflow-hidden" style={{ border: "1.5px solid #C8501A" }}>
            <input
              ...
              onKeyDown={(e) => e.key === "Enter" && handleSubscribe()}
              placeholder="Enter your Email Address"
              className="flex-1 min-w-0 bg-white text-xs text-gray-700 placeholder-gray-400 px-4 py-2.5 outline-none"
            />
            <button
              onClick={handleSubscribe}
