...
// ═══════════════════════════════════════════════════════════════════════════════
//  SIBLING ORDER LIST — FIXED
//  Robustly finds siblings even when parentId is not present in tree nodes.
// ═══════════════════════════════════════════════════════════════════════════════
function SiblingOrderList({ currentCategoryId, allCategories, onReordered }) {
  const [siblings, setSiblings] = useState([]);
  const [reordering, setReordering] = useState(false);
  ...
  useEffect(() => {
    ...
    // Find the current category anywhere in the tree
    const findCategory = (nodes, id) => {
      for (const node of nodes) {
        if (node.id === id) return node;
        if (node.children?.length) {
    ...
    // Determine parentId: use node.parentId if available, otherwise infer from tree structure
    let targetParentId = current.parentId || null;

    // If parentId is missing (tree builder may strip it), infer from tree structure
    if (targetParentId === undefined || targetParentId === null) {
      ...
      const inferredParent = findParent(allCategories, currentCategoryId);
      if (inferredParent !== undefined) {
        targetParentId = inferredParent;
      }
    }
    ...
    const collectSiblings = (nodes, parentId = null) => {
      nodes.forEach((node) => {
        const nodeParentId = node.parentId !== undefined ? node.parentId : parentId;
        // Compare parent IDs (handle both null and undefined)
        const sameParent =
          (targetParentId === null || targetParentId === undefined) &&
          (nodeParentId === null || nodeParentId === undefined)
            ? true
  ...
  const saveOrder = useCallback(async () => {
    ...
    try {
      ...
      setError(backendMsg || 'Failed to save category order.');
    } finally {
      setReordering(false);
    }
  }, [siblings, onReordered]);
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
            ...
            {/* ═══════════════════════════════════════════════════════════════════ */}
            {/*  Sibling Order Section (edit mode only)                          */}
            {/* ═══════════════════════════════════════════════════════════════════ */}
            {isEdit && (
              <div className="border-t border-gray-100 pt-4">
                <FormField
